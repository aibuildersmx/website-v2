...
> Copy-ready section skeletons. Every section on a marketing page should start from one of these five templates, then be filled with content. If your new section doesn't fit one of these, stop and check [validation.md](./validation.md) before inventing a new layout.
...
```mermaid
flowchart TD
    start["New section needed"] --> q1{"Is it the top of the page?"}
