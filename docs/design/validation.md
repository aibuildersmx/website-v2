...
### 3.6 `/blog` — external editorial destination

`/blog` and `/blog/*` now redirect to `https://aibuilders.lat`. Do not add local MDX content, blog route groups, blog components, or blog image assets in this repo.
