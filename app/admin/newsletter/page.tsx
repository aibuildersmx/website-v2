...
function StatusDot({ status }: { status: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      <span
        className={`h-1.5 w-1.5 rounded-full ${sent ? "bg-green-500" : "bg-black/20"}`}
      />
    ...
    <div>
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 font-serif text-3xl text-gray-800">Newsletter</h1>
        </div>
        <form action={createIssue}>
          <button
            type="submit"
            className="rounded-full bg-gray-900 px-5 py-2.5 font-mono text-[11px] uppercase tracking-[0.15em] text-white transition hover:bg-gray-700"
          >
            Nuevo issue
          </button>
        </form>
      ...
      <div className="mt-10 overflow-hidden rounded-2xl border border-black/5 bg-white">
        {issues.length === 0 ? (
              ...
              <li key={issue.id}>
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate font-serif text-lg text-gray-800">
                      {issue.subject || (
                        <span className="text-gray-300">Sin subject</span>
                      )}
                    </p>
                    <p className="mt-0.5 font-mono text-[10px] uppercase tracking-[0.2em] text-gray-400">
                      Issue {issue.slug}
                    </p>
                  </div>
